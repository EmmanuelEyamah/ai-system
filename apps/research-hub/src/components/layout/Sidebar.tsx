...
import { Plus, Search, Trash2, PanelLeftClose, PanelLeft, X, Star, LogOut } from "lucide-react";
...
function SessionItem({
  ...
  session: { id: string; title: string; status: string; starred: boolean };
  isActive: boolean;
  onNavigate: () => void;
    ...
    >
      <Search
        ...
        className={cn(
          ...
          isActive ? "text-cyan-400" : "text-zinc-700"
        )}
      />
      ...
      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-all">
        <button
          ...
          className={cn(
            ...
            session.starred ? "text-amber-400 opacity-100" : "text-zinc-700 hover:text-amber-400"
          )}
        >
...
export function Sidebar() {
  ...
  const { starredSessions, recentSessions, loading, deleteSession, toggleStar } = useResearchSessions();
  ...
  const currentSessionId = pathname?.split("/research/")[1];
  const isEmpty = starredSessions.length === 0 && recentSessions.length === 0;

  const sidebarContent = (
    <>
      <div className="flex items-center justify-between px-4 py-4">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-md bg-cyan-500/10 border border-cyan-500/15 flex items-center justify-center">
            <Search size={13} className="text-cyan-400" />
          </div>
          <span className="text-[13px] font-semibold text-zinc-300 tracking-tight">Research Hub</span>
        </div>
        <button
      ...
      <div className="px-3 mb-3">
        <motion.button
          ...
          className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-cyan-500/8 border border-cyan-500/10 text-cyan-400 text-[13px] font-medium hover:bg-cyan-500/12 hover:border-cyan-500/15 transition-smooth"
        >
          <Plus size={15} />
          New Research
        </motion.button>
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-3">
        {loading ? (
          ...
          >
            ...
            <p className="text-[13px] text-zinc-600 mb-1">No research yet</p>
            <p className="text-[11px] text-zinc-700">Start a new research session to begin</p>
          </motion.div>
        ) : (
          <>
                  ...
                  {starredSessions.map((s, i) => (
                    ...
                    >
                      <SessionItem
                        ...
                        onNavigate={() => handleSessionClick(s.id)}
      ...
      {collapsed && (
        ...
        >
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={handleNewResearch}
            className="p-2 rounded-lg bg-cyan-500/10 text-cyan-400 hover:bg-cyan-500/20 transition-smooth"
          >
            <Plus size={18} />
          </motion.button>
        </motion.div>
