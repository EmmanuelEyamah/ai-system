...
import { ChevronDown, Brain, Wand2, Sparkles, Zap, Crown } from "lucide-react";
import { MODELS, getModel, type ModelOption, type ModelSelection } from "@/lib/models";
...
function TierBadge({ tier }: { tier: ModelOption["tier"] }) {
  const config = {
    ...
    high: { icon: Sparkles, color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/15" },
...
export function ModelSelector({ selection, onChange }: ModelSelectorProps) {
  return (
    <div className="flex items-center gap-2">
      <ModelDropdown
        label="Analysis"
        icon={Brain}
        selectedId={selection.analysisModel}
        onSelect={(id) => onChange({ ...selection, analysisModel: id })}
      />
      <ModelDropdown
        label="Generation"
        icon={Wand2}
        selectedId={selection.generationModel}
        onSelect={(id) => onChange({ ...selection, generationModel: id })}
      />
    </div>
  );
}
