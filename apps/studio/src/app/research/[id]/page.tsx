...
import { ExportModal } from "@/modules/research/components/ExportModal";
...
export default function ResearchPage({
  ...
  const {
    events, report, status, query, title, starred, messages,
    loading, error, sending, fetchSession, startResearch, sendFollowUp,
  } = useResearch(id);
  ...
  // Internal navigation instead of cross-app handoff
  const handleGeneratePrompt = async (context: string) => {
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/chat/${data.chatId}?context=${encodeURIComponent(context)}`);
      }
    } catch {
      // silent
    }
    ...
    <AppShell>
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <motion.header
          ...
          className="border-b border-white/4 pl-14 pr-4 md:px-6 py-3"
        >
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              <div className={`w-2 h-2 rounded-full shrink-0 ${currentStatus.dot}`} />
              <span className={`text-[11px] sm:text-[12px] font-medium whitespace-nowrap ${currentStatus.color}`}>
                {currentStatus.label}
              </span>
            </div>
          ...
          <div className="max-w-3xl mx-auto px-6 py-8">
            <div className="mb-6">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[11px] uppercase tracking-widest text-zinc-600 font-semibold">Research Query</span>
              </div>
            ...
            {events.length > 0 && (
              <AgentActivityFeed events={events} isComplete={status === "completed" || status === "failed"} />
            )}
            ...
            {report && (
              <>
                <ReportRenderer report={report} onExport={() => setExportOpen(true)} onGeneratePrompt={handleGeneratePrompt} />
                <FollowUpChat messages={messages} sending={sending} onSendMessage={sendFollowUp} />
                <ExportModal report={report} sessionId={id} isOpen={exportOpen} onClose={() => setExportOpen(false)} />
              </>
            )}
          </div>
        </div>
