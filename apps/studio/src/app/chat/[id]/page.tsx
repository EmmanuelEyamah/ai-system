  ...
  return (
    <AppShell>
      <div className="flex-1 flex min-w-0 overflow-hidden relative">
        <div className={`flex-1 flex flex-col min-w-0 overflow-hidden ${showDesktopPanel ? "lg:max-w-[45%] lg:flex-none" : ""}`}>
          <div className="h-auto min-h-12 flex flex-wrap items-center justify-between gap-2 px-4 sm:px-6 pl-14 md:pl-6 py-2 border-b border-white/4">
            ...
            <div className="flex items-center gap-2">
              ...
              {hasPrompts && (
                <>
                  <button onClick={() => setPromptPanelOpen(true)}
                    ...
                    <Layers size={13} /> Prompts ({prompts.length})
                  </button>
                  {!desktopPanelOpen && (
                    <button onClick={() => setDesktopPanelOpen(true)}
                      ...
                      <Layers size={13} /> Show Prompts ({prompts.length})
                    </button>
                  )}
                </>
              )}
          ...
          <AnimatePresence>
            {showConfirmButtons && !sending && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
                ...
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={generatePrompts}
                  className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-black font-semibold text-[13px] transition-all glow-emerald">
                  <Sparkles size={15} /> Generate Prompts
                </motion.button>
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={addMore}
                  ...
                  <PlusCircle size={15} /> I want to add more
                </motion.button>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <ChatInput onSend={sendMessage} onRetry={retry} showRetry={!!error}
            disabled={sending || loading || showConfirmButtons}
            placeholder={showConfirmButtons ? "Click a button above to continue..." : messages.length === 0 ? "Paste your idea or describe what you need..." : "Reply or add more details..."}
          />
        </div>

        <AnimatePresence>
          {showDesktopPanel && (
            <motion.div initial={{ opacity: 0, x: 20, width: 0 }} animate={{ opacity: 1, x: 0, width: "55%" }}
              exit={{ opacity: 0, x: 20, width: 0 }} transition={{ duration: 0.3, ease: [0.25, 0.4, 0.25, 1] }}
              className="hidden lg:flex overflow-hidden">
              <PromptPanel prompts={prompts} onRegenerate={regeneratePrompts} onClose={() => setDesktopPanelOpen(false)} regenerating={sending} />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {promptPanelOpen && hasPrompts && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="lg:hidden fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" onClick={() => setPromptPanelOpen(false)}>
              <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }}
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-white/4">
                  ...
                  <button onClick={() => setPromptPanelOpen(false)} className="p-1.5 rounded-md hover:bg-white/5 text-zinc-500 hover:text-zinc-300 transition-smooth">
                    <X size={16} />
                  </button>
                </div>
